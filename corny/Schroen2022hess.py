...
def Field_at_Distance(R=0, theta1 = 0.05, theta2 = 0.10, hum = 5, bd=1.43, N0 = 1000, max_radius=500, resolution=0.01, verbose=True):
    """
    Function to calculate N_eff as a whole
    """
    N1 = sm2N_Koehli(theta1, h=hum, off=0.0, bd=bd, func='vers2', method='Mar21_uranos_drf', bio=0)*N0/N0_scaling
    N2 = sm2N_Koehli(theta2, h=hum, off=0.0, bd=bd, func='vers2', method='Mar21_uranos_drf', bio=0)*N0/N0_scaling
    ...
    W['wn'] = W.w/W.w.sum()
    W['a'] = np.nan
    W.loc[R:, 'a'] = 1/np.pi * np.arccos(R/W[R:].index)
    W['wa'] = W.wn * W.a
    w = W.wa.sum()
    N_eff = (1-w)*N1+w*N2
    ...
    if verbose:
        print('R={0:.1f}, theta1={1:.1%}, theta2={2:.1%}, theta_eff={3:.1%}, N1={4:.0f}, N2={5:.0f}, N_eff={6:.0f}, Influence={7:.1%}, Contribution={8:.1%}'.format(R, theta1, theta2, sm_eff, N1, N2, N_eff, w, w*N2/((1-w)*N1+w*N2)))
    return(N1, N2, N_eff, sm_eff, (1-w)*N1/((1-w)*N1+w*N2), w*N2/((1-w)*N1+w*N2))


def Plot_Field(ax=None, Var=None, R=0, footprint_radius=200, annotate=None, overlay=None,
               fontsize=10, title='Signal contribution', extent=500, cmap='Greys', vmax_factor=4, x_marker=None):
    """
    ...
    ax.imshow(Var, interpolation='none', cmap=cmap, vmax=np.max(Var)*vmax_factor, origin='lower', extent=(-extent2, extent2, -extent2, extent2))

    if annotate:
        ax.annotate('{0:.1%}'.format(annotate[0]), (-R2-extent4, 0), fontsize=fontsize, ha='center', va='center')
        ax.annotate('{0:.1%}'.format(annotate[1]), ( R2+extent4, 0), fontsize=fontsize, ha='center', va='center')
    ...
    # Distance marker
    if x_marker:
        ax.annotate('$R=%.0f\,$m' % R, xy=(R, extent2), xytext=(R, extent2*1.14),
    ...
    # Footprint circle
    fpc = plt.Circle((0, 0), footprint_radius, fc='none', ec='black', ls='--', alpha=0.3)
    ax.add_patch(fpc)
...
def Generate_Splitfields(size, R):
    size = size-1
    size2 = int(size/2)
    split_field = np.array([[0]*(size2+R)+[1]*(size2-R)], np.int8).repeat(size,0)
    return(split_field)

def Plot_Ntheta(ax, R, theta1, theta2, thetaeff, N1, N2, Neff, hum=5, off=0, bd=1.43, N0=1500):

    th = np.arange(0.01, 0.9, 0.01)
    N = [sm2N_Koehli(x, h=hum, off=off, bd=bd, func='vers2', method='Mar21_uranos_drf', bio=0)*N0/N0_scaling for x in th] 
    
    ax.plot(th, N, color='silver', lw=3, zorder=0)
    ax.scatter(theta1, N1, color='C1', marker='>', s=100, lw=2, fc='none',
               label=r'$\hat\theta_1$ = {0:4.1f}%,  $N_1$ = {1:4.0f} cph'.format(theta1*100, N1))
    ax.scatter(theta2, N2, color='C0', marker='<', s=100, lw=2, fc='none',
               label=r'$\hat\theta_2$ = {0:4.1f}%,  $N_2$ = {1:4.0f} cph'.format(theta2*100, N2))
    ax.scatter(thetaeff, Neff, color='k', marker='o', s=100, lw=2, fc='none',
               label=r'$\hat\theta$   = {0:4.1f}%,  $\hat N$  = {1:4.0f} cph'.format(thetaeff*100, Neff))
    ...
    ax.legend(ncol=1, fancybox=False, framealpha=1, edgecolor='none', handlelength=1, borderaxespad=1, loc='upper right')
...
def Plot_Sensibility(ax, N1, Neff):
    ...
    ax.annotate('${0:+.1%}$%'.format(Nrel), (0, np.abs(Nrel)*1.2), ha='center', fontsize=12)
...
def optimize_R(R, theta1, theta2, dN, N0, bd, verbose, max_radius):
